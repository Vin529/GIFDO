...
from config import (
    ...
    SNAPSHOT_EVERY_X_GENERATIONS, 
    POPULATION_SIZE, 
    TEMPERATURE_RANGE, 
    RADIATION_RANGE, 
    INITIAL_TEMPERATURE_MUTATION_STD, 
    INITIAL_RADIATION_MUTATION_STD, 
    CHECKPOINT_EVERY_X_GENERATIONS, 
    FIRST_CHECKPOINT_GENERATION
)
...
CHECKPOINT_RUN = "run_1"
CHECKPOINT_FILE_NAME = "checkpoint_generation_5.pickle"
...
def main():
    ...
    #either start from a checkpoint point from a prior run, or start a fresh run
    if LOAD_FROM_CHECKPOINT:
        ...
        history = {
            "top_fitness": [],
            "top_half_material_use_percentage": [],
            "top_half_fitness": [],
            "top_half_volume": [],
            "top_half_temperature_setpoint": [],
            "top_half_radiation_setpoint": [],
            "top_half_natural_radiation": [],
            "top_half_area_volume_ratio": [],
            "top_half_yield_value": [],
            "top_half_heating_cost": [],
            "top_half_cooling_cost": [],
            "top_half_lighting_cost": [],
            "top_half_window_wall_ratio": []
        }
...
#========================================== MAIN GENETIC ALGORITHM LOOP ==========================================
    for generation in range(start_generation, TOTAL_GENERATIONS + 1):
        ...
        if generation <= 100:
            ...
            #38 crossover for 50 candidates
            crossover_rate = 0.76
...
#=============================================================================================================
        ...
        history["top_fitness"].append(top_candidate.fitness)
        if generation % 10 == 0:
        ...
        history["top_half_fitness"].append([candidate.fitness for candidate in top_half_candidates])
        history["top_half_volume"].append([candidate.building_structure.mesh.volume for candidate in top_half_candidates])
        history["top_half_temperature_setpoint"].append([candidate.setpoints.temperature for candidate in top_half_candidates])
        history["top_half_radiation_setpoint"].append([candidate.setpoints.radiation for candidate in top_half_candidates])
        history["top_half_natural_radiation"].append([candidate.mean_daily_total_PAR_energy_per_area_MJ for candidate in top_half_candidates])
        history["top_half_area_volume_ratio"].append([candidate.building_structure.mesh.area / candidate.building_structure.mesh.volume for candidate in top_half_candidates])
        history["top_half_yield_value"].append([candidate.annual_yield_value_USD for candidate in top_half_candidates])
        history["top_half_heating_cost"].append([candidate.annual_heating_cost_USD for candidate in top_half_candidates])
        history["top_half_cooling_cost"].append([candidate.annual_cooling_cost_USD for candidate in top_half_candidates])
        history["top_half_lighting_cost"].append([candidate.annual_lighting_cost_USD for candidate in top_half_candidates])
        history["top_half_material_use_percentage"].append(_get_material_percentages(top_half_candidates))
        history["top_half_window_wall_ratio"].append(_get_window_wall_ratios(top_half_candidates))
        ...
        if generation % SNAPSHOT_EVERY_X_GENERATIONS == 0 and generation != TOTAL_GENERATIONS:
            ...
            _create_and_save_all_history_plots(save_directory=snapshot_directory, history=history)
            print(f"Summary data for generation {generation} saved to {snapshot_directory}")
        ...
        if (generation - FIRST_CHECKPOINT_GENERATION) % CHECKPOINT_EVERY_X_GENERATIONS == 0:
            _save_checkpoint(population=population, save_directory=this_run_directory, history=history)
    ...
    _create_and_save_all_history_plots(save_directory=final_results_directory, history=history)
...
def _save_checkpoint(population: Population, save_directory: str, history: dict) -> None:
    checkpoint_data = {
        "population": population,
        "history": history
    }
...
def _create_and_save_all_history_plots(save_directory: str, history: dict) -> None:
    if not os.path.exists(save_directory):
        raise FileNotFoundError(f"save directory not found: {save_directory}")

    save_top_fitness_history_plot(history["top_fitness"], save_directory)
    save_top_half_material_use_percentage_history_plot(history["top_half_material_use_percentage"], save_directory)

    #plot all the top half data graphs
    plot_configs = [
        (history["top_half_fitness"], "green", "Fitness", None),
        (history["top_half_volume"], "orange", "Volume", "m$^3$"),
        (history["top_half_temperature_setpoint"], "red", "Temperature Setpoint", "C"),
        (history["top_half_radiation_setpoint"], "#393b79", "PAR Setpoint", "MJ m$^{-2}$ day$^{-1}$"), #dark indigo
        (history["top_half_natural_radiation"], "purple", "Natural PAR", "MJ m$^{-2}$ day$^{-1}$"),
        (history["top_half_area_volume_ratio"], "blue", "Area/Volume Ratio", None),
        (history["top_half_yield_value"], "cyan", "Yield Value", "USD"),
        (history["top_half_heating_cost"], "pink", "Heating Cost", "USD"),
        (history["top_half_cooling_cost"], "brown", "Cooling Cost", "USD"),
        (history["top_half_lighting_cost"], "grey", "Lighting Cost", "USD"),
        (history["top_half_window_wall_ratio"], "olive", "Window/Wall Ratio", None)
    ]

    for data, colour, y_label, y_units in plot_configs:
